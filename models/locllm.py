...
from .dino import vit_large, vit_base
...
class LocLLMModel(LlamaForCausalLM):
    ...
    def __init__(self, config: LocLLMConfig):
        ...
        with lora_dino(r=config.lora_vision_r, alpha=config.lora_vision_alpha, dropout=config.lora_vision_dropout, enabled=config.lora_vision_enable):
            ...
            vision_model = vit_base(patch_size=14, img_size=518, drop_path_rate=0.4, drop_path_uniform=True, init_values=1.0, block_chunks=0)
            #vision_model = vit_base(embed_dim=768, num_heads=12, depth=12, patch_size=14, img_size=224, drop_path_rate=0.4, drop_path_uniform=True, init_values=1.0, block_chunks=0)
